...
DEFAULT_STUDENT_CKPT = os.path.join(CHECKPOINT_DIR, "student_cnn_kd_focal_best.pt")
...
class FocalLoss(nn.Module):
    """
    Multi-class focal loss using per-class weights.

    This uses the standard form:
        FL = alpha_t * (1 - p_t)^gamma * CE

    Where:
      - alpha_t is the optional per-class weight
      - p_t is the predicted probability of the true class
      - CE is standard cross-entropy for each sample
    """
...
def kd_loss(
    ...
    cls = cls_criterion(student_logits, targets)
    ...
    loss = alpha * cls + (1.0 - alpha) * kd
    return loss, cls.detach(), kd.detach()
...
def train_one_epoch(
    ...
    avg_cls = running_cls / float(total)
    avg_kd = running_kd / float(total)
    acc = float(correct) / float(total)

    return avg_loss, avg_cls, avg_kd, acc
...
def build_teacher_from_arch(teacher_arch: str, n_leads: int, n_classes: int) -> nn.Module:
    ...
    raise ValueError(
        f"Unsupported teacher_arch '{teacher_arch}' found in checkpoint. "
        "Expected one of: TeacherCNN, WeakTeacherCNN, StrongTeacherCNN."
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train KD student on PTB-XL with CE or Focal supervised loss"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight on supervised classification term",
    )
    ...
    parser.add_argument(
        ...
        default="ce",
        choices=["ce", "focal"],
        help="Supervised classification loss to use inside KD training",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Focusing parameter gamma for focal loss (only used when --loss_type focal)",
    )

    parser.add_argument(
        "--disable_class_weights",
        action="store_true",
        help="Disable inverse-frequency class weights for the supervised loss",
    )
...
def main():
    ...
    print(
        "KD hyperparameters: alpha={:.3f}, T={:.2f}, class_weight_gamma={:.2f}, "
        "student_size={}, loss_type={}, focal_gamma={:.2f}, class_weights_enabled={}".format(
            args.alpha,
            args.temperature,
    ...
    if not os.path.exists(args.teacher_ckpt):
        raise FileNotFoundError(
            f"Teacher checkpoint not found at {args.teacher_ckpt}. Train the teacher first."
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_cls, train_kd, train_acc = train_one_epoch(
            teacher,
            student,
            train_loader,
            cls_criterion,
        ...
        print(
            "[Epoch {:02d}] train_loss={:.4f}, train_cls={:.4f}, train_kd={:.4f}, "
    ...
    print("Loss type:", kd_ckpt.get("loss_type", "UNKNOWN"), flush=True)
    if kd_ckpt.get("loss_type", "") == "focal":
        print("Focal gamma:", kd_ckpt.get("focal_gamma", "UNKNOWN"), flush=True)
