...
def compute_class_frequencies(labels: np.ndarray) -> np.ndarray:
    ...
    return class_counts.astype(np.float32) / class_counts.sum()


def compute_kd_class_weights(labels: np.ndarray, beta: float) -> torch.Tensor:
    freq = compute_class_frequencies(labels)
    inv = 1.0 / (freq + 1e-6)
    weights = np.power(inv, beta)
    weights = weights / weights.mean()
    return torch.tensor(weights, dtype=torch.float32)
...
def kd_loss(
    ...
    kd_weight_mode: str,
    kd_class_weights: torch.Tensor = None,
):
    ...
    if kd_weight_mode == "none":
        soft_loss = soft_loss_per_sample.mean()
        kd_weight_mean = torch.tensor(1.0, device=student_logits.device)
    elif kd_weight_mode == "class_balanced":
        if kd_class_weights is None:
            raise ValueError("kd_class_weights is required when kd_weight_mode='class_balanced'")
        sample_kd_weights = kd_class_weights[targets].to(student_logits.device, dtype=student_logits.dtype)
        soft_loss = (sample_kd_weights * soft_loss_per_sample).mean()
        kd_weight_mean = sample_kd_weights.mean().detach()
    else:
        raise ValueError(f"Unsupported kd_weight_mode: {kd_weight_mode}")
...
def train_one_epoch(
    ...
    for X, y in loader:
        ...
        with torch.amp.autocast(device_type="cuda", enabled=use_amp):
            ...
            loss, hard_loss, soft_loss, logit_mse, feature_mse, kd_weight_mean = kd_loss(
        ...
        running_kd_weight_mean += kd_weight_mean.item() * X.size(0)
    ...
    avg_kd_weight_mean = running_kd_weight_mean / float(total)
    acc = float(correct) / float(total)

    return avg_loss, avg_hard, avg_soft, avg_logit_mse, avg_feature_mse, avg_kd_weight_mean, acc


def parse_args():
    parser = argparse.ArgumentParser(description="Train KD student on PTB-XL with minority-aware KD")
    ...
    parser.add_argument("--kd_weight_beta", type=float, default=0.35)
...
def main():
    ...
    print(
        (
            ...
            f"kd_weight_mode={args.kd_weight_mode}, "
            f"kd_weight_beta={args.kd_weight_beta:.3f}"
        ),
        flush=True,
    )
    print(
    ...
    kd_class_weights = None
    if args.kd_weight_mode == "class_balanced":
        kd_class_weights = compute_kd_class_weights(
            np.array(y_train),
            beta=args.kd_weight_beta,
        ).to(device)
        print("KD class weights:", kd_class_weights.cpu().numpy(), flush=True)
        for idx, cls_name in enumerate(classes):
            print(
                f"  class={cls_name} kd_weight={kd_class_weights[idx].item():.4f}",
                flush=True,
            )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_hard, train_soft, train_logit_mse, train_feature_mse, train_kd_weight_mean, train_acc = train_one_epoch(
            teacher=teacher,
            student=student,
            teacher_projector=teacher_projector,
            loader=train_loader,
            ...
            torch.save(
                {
                    ...
                    "feature_mse_weight": args.feature_mse_weight,
                    "kd_weight_mode": args.kd_weight_mode,
                    "kd_weight_beta": args.kd_weight_beta,
    ...
    print("KD weight mode:", ckpt.get("kd_weight_mode", "UNKNOWN"), flush=True)
