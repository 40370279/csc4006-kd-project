...
def compute_class_frequencies(labels: np.ndarray) -> np.ndarray:
    class_counts = np.bincount(labels)
    class_freq = class_counts.astype(np.float32) / class_counts.sum()
    return class_freq


def compute_adaptive_alpha_per_class(
    labels: np.ndarray,
    alpha_min: float,
    alpha_max: float,
) -> torch.Tensor:
    class_freq = compute_class_frequencies(labels)

    freq_min = float(class_freq.min())
    freq_max = float(class_freq.max())

    if np.isclose(freq_min, freq_max):
        alpha_per_class = np.full_like(class_freq, fill_value=alpha_max, dtype=np.float32)
    else:
        freq_norm = (class_freq - freq_min) / (freq_max - freq_min)
        alpha_per_class = alpha_min + freq_norm * (alpha_max - alpha_min)

    return torch.tensor(alpha_per_class, dtype=torch.float32)
...
def kd_loss(
    ...
    alpha_mode: str,
    adaptive_alpha_per_class: torch.Tensor = None,
):
    ...
    elif alpha_mode == "classwise_adaptive":
        if adaptive_alpha_per_class is None:
            raise ValueError("adaptive_alpha_per_class is required for classwise_adaptive mode")
        alpha_vec = adaptive_alpha_per_class[targets].to(student_logits.device, dtype=student_logits.dtype)
    else:
        raise ValueError(f"Unsupported alpha_mode: {alpha_mode}")

    mixed_main_loss = (alpha_vec * hard_loss_per_sample + (1.0 - alpha_vec) * soft_loss_per_sample).mean()
    ...
    return (
        total_loss,
        hard_loss_per_sample.mean().detach(),
        soft_loss_per_sample.mean().detach(),
        logit_mse.detach(),
        feature_mse.detach(),
        alpha_vec.mean().detach(),
    )
...
def train_one_epoch(
    ...
    logit_mse_weight: float,
    alpha_mode: str,
    adaptive_alpha_per_class: torch.Tensor,
):
    teacher.eval()
    student.train()
    ...
    for X, y in loader:
        ...
        with torch.amp.autocast(device_type="cuda", enabled=use_amp):
            ...
            loss, hard_loss, soft_loss, logit_mse, feature_mse, alpha_mean = kd_loss(
                ...
                adaptive_alpha_per_class=adaptive_alpha_per_class,
            )
        ...
        running_alpha += alpha_mean.item() * X.size(0)
    ...
    avg_alpha = running_alpha / float(total)
    acc = float(correct) / float(total)

    return avg_loss, avg_hard, avg_soft, avg_logit_mse, avg_feature_mse, avg_alpha, acc


def parse_args():
    parser = argparse.ArgumentParser(description="Train KD student on PTB-XL with class-aware adaptive alpha")
    ...
    parser.add_argument(
        ...
        type=str,
        default="fixed",
        choices=["fixed", "classwise_adaptive"],
    )
    parser.add_argument("--adaptive_alpha_min", type=float, default=0.05)
    parser.add_argument("--adaptive_alpha_max", type=float, default=0.30)
...
def main():
    ...
    print(
        (
            ...
            f"adaptive_alpha_min={args.adaptive_alpha_min:.3f}, "
            f"adaptive_alpha_max={args.adaptive_alpha_max:.3f}"
        ),
        flush=True,
    )
    print(
    ...
    adaptive_alpha_per_class = None
    if args.alpha_mode == "classwise_adaptive":
        adaptive_alpha_per_class = compute_adaptive_alpha_per_class(
            np.array(y_train),
            alpha_min=args.adaptive_alpha_min,
            alpha_max=args.adaptive_alpha_max,
        ).to(device)
        print("Adaptive alpha per class:", adaptive_alpha_per_class.cpu().numpy(), flush=True)
        for idx, cls_name in enumerate(classes):
            print(
                f"  class={cls_name} alpha={adaptive_alpha_per_class[idx].item():.4f}",
                flush=True,
            )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_hard, train_soft, train_logit_mse, train_feature_mse, train_alpha_mean, train_acc = train_one_epoch(
            teacher=teacher,
            student=student,
            teacher_projector=teacher_projector,
            loader=train_loader,
        ...
        print(
            ...
            "train_logit_mse={:.4f}, train_feature_mse={:.4f}, train_acc={:.4f}, "
            "alpha_scalar={:.3f}, train_alpha_mean={:.3f}, "
    ...
    print("Alpha mode:", ckpt.get("alpha_mode", "UNKNOWN"), flush=True)
